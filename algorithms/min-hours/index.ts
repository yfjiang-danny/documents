...
解释：你不需要额外的精力和经验就可以赢得比赛，所以返回 0 。
 ...
 == energy.length == experience.length
1 <= n <= 100
  ...
  export function minNumberOfHours(
    ...
    for (let i = 0; i < n; i++) {
      if (curEnergy <= energy[i]) {
        const need = energy[i] - curEnergy + 1;
        minEnergy += need;
        curEnergy += need;
      }
      curEnergy -= energy[i];
      if (curExperience <= experience[i]) {
        const need = experience[i] - curExperience + 1;
        minExperience += need;
        curExperience += need;
      }
...
(() => {
  ...
  console.log("result:", Easy.minNumberOfHours(input1, input2, input3, input4));
})();
